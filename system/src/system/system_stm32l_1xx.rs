...
pub fn system_clock_set_default() {
    ...
    reg_value_clear_mask(RCC_CR as *mut u32, 0xEEFEFFFE);
    
    /*< Reset HSEBYP bit */
    reg_value_clear_bit(RCC_CR  as *mut u32, RCC_CR_HSEBYP_Pos);

    /*< Reset PLLSRC, PLLMUL[3:0] and PLLDIV[1:0] bits */
    reg_value_clear_mask(RCC_CFGR  as *mut u32, 0xFF02FFFF);
...
pub fn system_init() {
    if VALUE_SVCORE == VC_1200 {
        if FOSC_KHZ_VALUE > 2000 {
            reg_value_set_bit(FLASH_ACR as *mut u32, FLASH_ACR_ACC64_Pos);
    ...
    reg_value_set(RCC_CFGR as *mut u32, VALUE_RCC_CFGR);             /* set clock configuration register */
    reg_value_set(RCC_CR as *mut u32, VALUE_RCC_CR & 0x000FFFFF);    /* do not start PLLs yet */
